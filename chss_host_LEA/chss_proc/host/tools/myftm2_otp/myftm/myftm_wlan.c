...
#define FTM_WLAN_LOAD_CMD       "/system/bin/insmod " \
                                "/sbin/insmod wlan.ko " MODE_PARAM "=5"
...
int32_t WlanATWriteOtp()
{
	return qca6174WriteOTP();
}

int32_t WlanATReadOtp()
{
	return qca6174ReadOTP();
}

void WlanATSetOtpAddr(int32_t otp_addr)
{
	DPRINTF(FTM_DBG_TRACE, "%s: otp_addr %x\n", __func__, otp_addr);
	qca6174SetOtpAddr(otp_addr);
}

void WlanATSetOtpDataLen(int32_t otp_data_len)
{
	DPRINTF(FTM_DBG_TRACE, "%s: WlanATSetOtpDataLen\n", __func__);
	qca6174SetOtpDataLen(otp_data_len);
}

void WlanATSetOtpData(int8_t *otp_data)
{
	DPRINTF(FTM_DBG_TRACE, "%s: RSSI self test\n", __func__);
	qca6174SetOtpData(otp_data);
}
