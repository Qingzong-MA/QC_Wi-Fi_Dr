...
typedef struct _otp_data_struct{
	union{
        long long input_val;
        char mac[8];
	}u_otp_data;
...
int main(int argc, char *argv[])
{
	...
	static struct option options[] = {
		...
		{"otp_read", no_argument, NULL, MYFTM_OTP_CMD_READ},
		{"otp_write", required_argument, NULL, MYFTM_OTP_CMD_WRITE},
		{"otp_addr", required_argument, NULL, MYFTM_OTP_ADDR},
		{"otp_data_len", required_argument, NULL, MYFTM_OTP_DATA_LEN},
		{0, 0, 0, 0}
	};
	int daemonize = 0;
	int int_parm = 0;
	...
	while (1) {
		c = getopt_long(argc, argv,
				...
				":lpwfmsk:ulofdmat"
				"otp_read:otp_write:otp_addr:otp_data_len ",options, NULL);
		...
		switch (c) {
			...
			case MYFTM_OPT_CMD_RST:
				...
				WlanATCmd_RST();
				break;
			case MYFTM_OTP_CMD_READ:
				WlanATReadOtp();
				break;
			case MYFTM_OTP_CMD_WRITE:
				{
					long long mac_v = 0;
					char *endptr;
					OTP_DATA_STRUCT otp_mac;

					otp_mac.u_otp_data.input_val = 0;
					otp_mac.u_otp_data.input_val = strtoll(optarg, &endptr, 16);

					WlanATSetOtpData(otp_mac.u_otp_data.mac);
					if(WlanATWriteOtp())
						printf("write OTP failed\n");
					break;	
				}
			case MYFTM_OTP_ADDR:
				{
					char *endptr;
					int32_t otp_addr = 0;
					otp_addr = strtol(optarg, &endptr, 16);
					if (*endptr != '\0') {
						printf("please enter otp addr as 0xXXXX\n");
					}
					WlanATSetOtpAddr(otp_addr);
					break;
				}
			case MYFTM_OTP_DATA_LEN:
				WlanATSetOtpDataLen(atoi(optarg));
				break;
