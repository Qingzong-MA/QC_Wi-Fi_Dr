/*
 * Copyright (c) 2014-2024 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
#define EEPROM_READ_BLOCK_SIZE    4096
	...
	uint32_t len = 0;
	uint32_t offset = 0;
	uint16_t blksize = EEPROM_READ_BLOCK_SIZE;
	uint32_t bdsize = 0;
...
int qca6174EepromWrite(char *val)
{
	...
	uint32_t offset = 0;
	uint16_t blksize = EEPROM_READ_BLOCK_SIZE;
	uint16_t data[EEPROM_READ_BLOCK_SIZE];
	uint32_t bdsize = 0;
	...
	{
		if (gCmd.Cmprsflag ==1 && g_eeprom_size<EEPROM_READ_BLOCK_SIZE)
		{
