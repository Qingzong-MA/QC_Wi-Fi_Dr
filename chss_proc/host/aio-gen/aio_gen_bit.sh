...
while getopts ":t:w:b:i:hevVrs" optname
  do
    case "$optname" in
      "t")
      ...
      "e")
        ...
        ;;
      "s")
        #echo "Option $optname is specified, apply emulation patches"
        EMULATION_PATCH=1
        ENG_PATCH=1
        ;;
      "v")
...
#For EasyMesh-R1 (Multi-AP) Support
BUILD_EMESH_STACK=n
...
then
    ...
    if [ $ENG_PATCH == 0 ]
    then
        echo " Downloading patch files from CLO"
        # Common patches
        ...
        then
            ...
            for i in ${tmp_arr[@]}
            do
                wget ${BOARD_TYPE_AIO_PATCH_CLO}/"$i"
        ...
        elif [ "$DOWNLOAD_WLAN_PATCH_PROJECT" == "y" ]
        then
            mkdir -p ${TOPDIR}/drivers/patches/${INPUT_BOARD_TYPE}
            git clone ${WLAN_PATCH_GIT_PATH} -b ${WLAN_PATCH_BRANCH} wlan_patches
            cd wlan_patches
            if [ -n "${CUTOFF_ID_WLAN_PATCH_CLO}" ]
            then
                git reset --hard ${CUTOFF_ID_WLAN_PATCH_CLO}
            fi
            ...
            then
                cp -rf ./fixce/3rdparty/patches/${INPUT_BOARD_TYPE}/* ${TOPDIR}/drivers/patches/${INPUT_BOARD_TYPE}
            fi
            #cp -rf ./fixce/3rdparty/patches/wlan_patches/supplicant ${TOPDIR}/drivers/patches/${INPUT_BOARD_TYPE}
            #cp -rf ./fixce/3rdparty/patches/wlan_patches/sigma_dut ${TOPDIR}/drivers/patches/${INPUT_BOARD_TYPE}
            cd ..
            rm -rf wlan_patches
        fi
        ...
        echo " Downloading patch files from local"
...
#
# Download core tech modules
#
if [ "$DOWNLOAD_CORE_TECH_MODULES" == "y" ]
then
    echo "========================================================"
    echo " Downloading core tech modules ..."
    echo "========================================================"
    if [ -d "${AIO_TOP}/drivers/core_tech_modules" ]
    then
        rm -rf ${AIO_TOP}/drivers/core_tech_modules
    fi
    cd ${AIO_TOP}/drivers
    if [ $ENG_PATCH -eq 1 ]; then
        git clone ${CORE_TECH_GIT_PATH} -b ${CORE_TECH_BRANCH} core_tech_modules

        if [ "$CUTOFF_ID_CORE_TECH" != "" ]
        then
            cd core_tech_modules
            git reset --hard ${CUTOFF_ID_CORE_TECH}
        fi
        for i in "${CORE_TECH_GERRITS_LIST[@]}"
        do
            git fetch ${GIT_REVIEW_SERVER}/platform/vendor/qcom-opensource/wlan-cnss-core refs/changes/${i} && git cherry-pick FETCH_HEAD
        done
        if [ $EMULATION_PATCH -eq 1 ]; then
            for i in "${CORE_TECH_EMULATION_PATCHES[@]}"
            do
                git fetch ${GIT_REVIEW_SERVER}/platform/vendor/qcom-opensource/wlan-cnss-core refs/changes/${i} && git cherry-pick FETCH_HEAD
            done
        fi
    else
        git clone ${CORE_TECH_GIT_PATH_CLO} -b ${CORE_TECH_BRANCH_CLO} core_tech_modules

        if [ "$CUTOFF_ID_CORE_TECH_CLO" != "" ]
        then
            cd core_tech_modules
            git reset --hard ${CUTOFF_ID_CORE_TECH_CLO}
        fi
    fi
fi

#
# Download CLD3.2 driver source code
#
if [ "$DOWNLOAD_DRIVER_WLAN_HOST_CLD3_2" == "y" ]
then
    if [ -d "${AIO_TOP}/drivers/qcacld-3.0" ]
    then
        rm -rf ${AIO_TOP}/drivers/qcacld-3.0
    fi

    if [ -d "${AIO_TOP}/drivers/qca-wifi-host-cmn" ]
    then
        rm -rf ${AIO_TOP}/drivers/qca-wifi-host-cmn
    fi

    if [ -d "${AIO_TOP}/drivers/fw-api" ]
    then
        rm -rf ${AIO_TOP}/drivers/fw-api
    fi

    echo "Download the qcacld-3.0 for wlan driver"

    cd ${AIO_TOP}/drivers
    if [ $ENG_PATCH -eq 1 ]; then
        git clone ${GIT_SERVER}/platform/vendor/qcom-opensource/wlan/qcacld-3.0.git -b ${CLD3_BRANCH}
        if [ "$CUTOFF_ID_CLD3" != "" ]
        then
            cd qcacld-3.0
            git reset --hard ${CUTOFF_ID_CLD3}
        fi

        for i in "${CLD3_GERRITS_LIST[@]}"
        do
            git fetch ${GIT_REVIEW_SERVER}/platform/vendor/qcom-opensource/wlan/qcacld-3.0 refs/changes/${i} && git cherry-pick FETCH_HEAD
        done
        if [ $EMULATION_PATCH -eq 1 ]; then
            for i in "${CLD3_EMULATION_PATCHES[@]}"
            do
                git fetch ${GIT_REVIEW_SERVER}/platform/vendor/qcom-opensource/wlan/qcacld-3.0 refs/changes/${i} && git cherry-pick FETCH_HEAD
            done
        fi
    else
        git clone ${CLD3_GIT_PATH_CLO} -b ${CLD3_BRANCH_CLO}
        if [ "$CUTOFF_ID_CLD3_CLO" != "" ]
        then
            cd qcacld-3.0
            git reset --hard ${CUTOFF_ID_CLD3_CLO}
        fi
        for i in "${CLD3_GERRITS_LIST_CLO[@]}"
        do
            git fetch ${CLD3_GIT_PATH_CLO} && git cherry-pick ${i}
        done
    fi

    echo "Download the qca-wifi-host-cmn for wlan driver"
    cd ${AIO_TOP}/drivers
    if [ $ENG_PATCH -eq 1 ]; then
        git clone ${GIT_SERVER}/platform/vendor/qcom-opensource/wlan/qca-wifi-host-cmn.git -b ${CMN_BRANCH}
        if [ "$CUTOFF_ID_CMN" != "" ]
        then
            cd qca-wifi-host-cmn
            git reset --hard ${CUTOFF_ID_CMN}
        fi

        for i in "${CMN_GERRITS_LIST[@]}"
        do
            git fetch ${GIT_REVIEW_SERVER}/platform/vendor/qcom-opensource/wlan/qca-wifi-host-cmn refs/changes/${i} && git cherry-pick FETCH_HEAD
        done
        if [ $EMULATION_PATCH -eq 1 ]; then
            for i in "${CMN_EMULATION_PATCHES[@]}"
            do
                git fetch ${GIT_REVIEW_SERVER}/platform/vendor/qcom-opensource/wlan/qca-wifi-host-cmn refs/changes/${i} && git cherry-pick FETCH_HEAD
            done
        fi
    else
        git clone ${CMN_GIT_PATH_CLO} -b ${CMN_BRANCH_CLO}
        if [ "$CUTOFF_ID_CMN_CLO" != "" ]
        then
            cd qca-wifi-host-cmn
            git reset --hard ${CUTOFF_ID_CMN_CLO}
        fi
        for i in "${CMN_GERRITS_LIST_CLO[@]}"
        do
            git fetch ${CMN_GIT_PATH_CLO} && git cherry-pick ${i}
        done
    fi

    echo "Download the fw-api for wlan driver"
    cd ${AIO_TOP}/drivers
    if [ $ENG_PATCH -eq 1 ]; then
        git clone ${GIT_SERVER}/platform/vendor/qcom-opensource/wlan/fw-api.git -b ${FWAPI_BRANCH}
        if [ "$CUTOFF_ID_FWAPI" != "" ]
        then
            cd fw-api
            git reset --hard ${CUTOFF_ID_FWAPI}
        fi

        for i in "${FWAPI_GERRITS_LIST[@]}"
        do
            git fetch ${GIT_REVIEW_SERVER}/platform/vendor/qcom-opensource/wlan/fw-api refs/changes/${i} && git cherry-pick FETCH_HEAD
        done
    else
        git clone ${FWAPI_GIT_PATH_CLO} -b ${FWAPI_BRANCH_CLO}
        if [ "$CUTOFF_ID_FWAPI_CLO" != "" ]
        then
            cd fw-api
            git reset --hard ${CUTOFF_ID_FWAPI_CLO}
        fi
    fi
fi
...
if [ -f ${AIO_TOP}/drivers/patches/${INPUT_BOARD_TYPE}/wlan/WCNSS_qcom_cfg.usb.ini ]
then
        cp ${AIO_TOP}/drivers/patches/${INPUT_BOARD_TYPE}/wlan/WCNSS_qcom_cfg.usb.ini ${AIO_TOP}/drivers/qcacld-new/firmware_bin
fi

if [ -f ${AIO_TOP}/drivers/patches/${INPUT_BOARD_TYPE}/wlan/WCNSS_qcom_cfg.ini ]
then
        mkdir -p ${AIO_TOP}/drivers/qcacld-3.0/firmware_bin
        cp ${AIO_TOP}/drivers/patches/${INPUT_BOARD_TYPE}/wlan/WCNSS_qcom_cfg.ini ${AIO_TOP}/drivers/qcacld-3.0/firmware_bin
fi
...
#Append the WLAN_BUILD_VER to QWLAN_VERSIONSTR in CORE/MAC/inc/qwlan_version.h
#
if [ "$WLAN_BUILD_VER" != "" ] && [ -f  ${AIO_TOP}/drivers/qcacld-3.0/core/mac/inc/qwlan_version.h ];
then
    echo "========================================================"
    echo " Append the WLAN_BUILD_VER to QWLAN_VERSIONSTR in qcacld-3.0/core/mac/inc/qwlan_version.h"
    echo "========================================================"
    qwlan_build_version=`grep "QWLAN_VERSIONSTR"  ${AIO_TOP}/drivers/qcacld-3.0/core/mac/inc/qwlan_version.h| awk -v ver=$WLAN_BUILD_VER  '{printf("%s\".%s\"", $3,ver)}'`
    sed -i 's/QWLAN_VERSIONSTR\s\+\".*\"/QWLAN_VERSIONSTR             '''$qwlan_build_version'''/' ${AIO_TOP}/drivers/qcacld-3.0/core/mac/inc/qwlan_version.h
fi
...
then
    ...
    git reset --hard ${CUTOFF_ID_HOSTAP}
    for i in "${REVERT_ID_HOSTAP[@]}"
    do
       git revert --no-edit ${i}
    done
    if ls ${AIO_TOP}/drivers/patches/${INPUT_BOARD_TYPE}/supplicant/*.patch >/dev/null 2>&1
    then
        git am ${AIO_TOP}/drivers/patches/${INPUT_BOARD_TYPE}/supplicant/*.patch
    fi
    ...
    if [ "$BUILD_EMESH_STACK" == "y" ]
    then
        wget https://git.codelinaro.org/clo/sba-patches/wlan_patches/-/raw/caf_migration/master/fixce/3rdparty/patches/ve-f10/ezmesh/hostapd/0001-Disable-bridge-interface-and-support-libwpa_client.s.patch
	git am 0001-Disable-bridge-interface-and-support-libwpa_client.s.patch
    fi
fi
...
then
    ...
    echo "===================================="
    git clone ${SIGMA_DUT_GIT_PATH_CLO} -b ${SIGMA_DUT_BRANCH_CLO}
    mv -f sigma-dut ${AIO_TOP}/apps
    cd ${AIO_TOP}/apps/sigma-dut
    git reset --hard ${CUTOFF_ID_SIGMA_DUT}
    if [ -d "${AIO_TOP}/drivers/patches/${INPUT_BOARD_TYPE}/sigma_dut" ]
    then
        git am ${AIO_TOP}/drivers/patches/${INPUT_BOARD_TYPE}/sigma_dut/*.patch
    fi

    if [ $? -ne 0 ]
    then
         echo "Error: Applying sigma patches fail!"
    fi
    cd -
fi
...
if [ "$BUILD_EMESH_STACK" == "y" ]
then
    EZMESHDIR=${AIO_TOP}/emesh_source
    EZMESHSRC=${ENGPATCHDIR}/${INPUT_BOARD_TYPE}/ezmesh_scripts
    if [ -d "${EZMESHDIR}" ]; then
        echo "EasyMesh Source directory exist (${EZMESHDIR}). Delete or move it"
    else
        echo "===================================="
        echo " Clone EasyMesh source ..."
        echo "===================================="

        EZMESHPATCH=${EZMESHSRC}/ezm_patches
        if [ -d ${EZMESHSRC} ]; then
            . ${EZMESHSRC}/ezm_src_clone.sh ${EZMESHDIR} ${EZMESHPATCH}
            cp -f ${EZMESHSRC}/Makefile ${EZMESHDIR}
	else
	    echo "Error: EasyMesh scripts and patches not available!!"
	fi
        cd ${TOPDIR}
    fi
fi
...
echo "export BOARD_TYPE_AIO_PATCH_CLO=${BOARD_TYPE_AIO_PATCH_CLO}"
echo "export IF_TYPE=${IF_TYPE}"
echo "export ENG_PATCH=${ENG_PATCH}"
echo "export WLAN_BUILD_VER=${WLAN_BUILD_VER}"
if [ "$BUILD_EMESH_STACK" == "y" ]; then
echo "export BUILD_EMESH_STACK=y"
echo "export EZMESHDIR=${EZMESHDIR}"
echo "export EZMESHSRC=${EZMESHSRC}"
fi
exec 1>&6 6>&-
