...
[bt_le_audio]="bt_le_audio_codebase" \
)
...
if [[ $1 == "d" ]]; then
	...
	if [ -d ${FIXCE_BT_PATCH_DIR}/bt_le_audio ] ; then
		cp ${FIXCE_BT_PATCH_DIR}/bt_le_audio ./ -rf
		pushd bt_le_audio
		git init .
		git add .
		git commit -m "code base from bt-vndr.lnx.13.0"
		git tag -a -m "bt_le_audio codebase" bt_le_audio_codebase
		popd
	fi
	...
	do_patch_helper fluoride "${FIXCE_BT_PATCH_DIR}/fluoride"
