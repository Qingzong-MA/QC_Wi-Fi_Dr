...
void handleEFUSEReadRSP (void *parms)
{
	...
	if (pParms->status != 0)
	{
		printf("OTP read returns error %d\n", pParms->status);
		return;
	}
	printf("status = %u\n", pParms->status);
	printf("numBytes = %u\n", pParms->numBytes);
	for(ii = 0; ii < pParms->numBytes; ii++){
		printf("otp_data = 0x%02x\n", pParms->otp_data[ii]);
	}
...
int qca6174ReadOTP(void)
{
	...
	ALOGE("\n %s() TLV2 CMD_EFUSEREAD %d", __func__, gCmd.otp_data_len);

	registerOTPREADRSPHandler(handleEFUSEReadRSP);
	tlv2CreateCmdHeader(CMD_EFUSEREAD);

	tlv2AddParms(2, PARM_OFFSET, gCmd.otp_addr);
	
	tlv2AddParms(2, PARM_NUMBYTES, gCmd.otp_data_len);
...
int qca6174WriteOTP(void)
{
	...
	uint32_t cmdStreamLen = 0;
	int i = 0;
	uint8_t tmp_data[ATH_MAC_LEN];
	
	if(gCmd.otp_data_len == 0){
		return -1;
	}
	...
	ALOGE("\n %s() TLV2 CMD_EFUSEWRITE %d", __func__, gCmd.otp_data_len);

	registerOTPWRITERSPHandler(handleEFUSEWriteRSP);
	...
	if(gCmd.otp_data_len > 1){
		memset(tmp_data, 0, ATH_MAC_LEN);
		memcpy(tmp_data, gCmd.otp_data, gCmd.otp_data_len);
		
		for(i = 0; i < gCmd.otp_data_len; i++){
			gCmd.otp_data[i] = tmp_data[gCmd.otp_data_len-i-1];
		}
	}
	
	tlv2AddParms(4, PARM_EFUSEDATA, gCmd.otp_data_len, 0, gCmd.otp_data);
...
void qca6174SetOtpAddr(uint32_t OtpAddr)
{
	gCmd.otp_addr = OtpAddr;
}

void qca6174SetOtpDataLen(uint32_t OtpDataLen)
{
	gCmd.otp_data_len = OtpDataLen;
}

void qca6174SetOtpData(uint8_t *otp_data)
{
	if(otp_data)
		memcpy(gCmd.otp_data, otp_data, ATH_MAC_LEN);
}
...
/* This is a OFDMA toneplan response callback handler register with
...
/* This function configured OFDMA toneplan parameter using
...
{
	gCmd.phyid_mask = PHYID_MASK_DEFAULT;

	if (val == PHYID_MASK_0)
	    gCmd.phyid_mask = PHYID_MASK_0;
	else if (val == PHYID_MASK_1)
	    gCmd.phyid_mask = PHYID_MASK_1;
	else if (val == PHYID_MASK_0_1)
	    gCmd.phyid_mask = PHYID_MASK_0_1;
	else
	    ALOGE("\n %s() Error to set PHYID_MASK", __func__);
}
...
int qca6174Cmd_LOWPOWER()
{
	...
	ALOGE("\n %s() TLV2 CMD_LOWPOWER Config power mode %d"
		" phyidmask %d and feature_mask %d \n ",
		 __func__, gCmd.lopwr_mode, gCmd.phyid_mask,gCmd.lpwr_fwmask);
