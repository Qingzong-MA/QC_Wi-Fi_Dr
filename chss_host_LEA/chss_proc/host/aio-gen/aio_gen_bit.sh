...
then
    ...
    then
        ...
        elif [ "$DOWNLOAD_AIO_PATCH_PROJECT" == "y" ]
        then
            mkdir -p ${TOPDIR}/drivers/patches/${INPUT_BOARD_TYPE}
            git clone ${AIO_PATCH_GIT_PATH} -b ${AIO_PATCH_BRANCH} wlan_patches
            cd wlan_patches
            if [ -n "${CUTOFF_ID_AIO_PATCH}" ]
            then
...
then
    ...
    echo "===================================="
    git clone ${SIGMA_DUT_GIT_PATH} -b ${SIGMA_DUT_BRANCH}
