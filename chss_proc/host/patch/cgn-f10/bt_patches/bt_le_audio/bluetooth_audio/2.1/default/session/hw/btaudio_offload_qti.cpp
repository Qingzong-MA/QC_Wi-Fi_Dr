...
#undef LOG_TAG
#endif
#define LOG_TAG "btaudio_offload_qti_2_1"
...
#define STREAM_RX_ONLY_CONFIG    0x1
#define STREAM_TX_ONLY_CONFIG    0x2
#define STREAM_TX_RX_BOTH_CONFIG 0x3
...
using CodecConfiguration = ::vendor::qti::hardware::bluetooth_audio::V2_1::CodecConfiguration;
using AacData = ::vendor::qti::hardware::bluetooth_audio::V2_0::AacParameters;
using LdacData = ::vendor::qti::hardware::bluetooth_audio::V2_0::LdacParameters;
using CodecConfiguration_2_0 = ::vendor::qti::hardware::bluetooth_audio::V2_0::CodecConfiguration;
using CodecType_2_0 = ::vendor::qti::hardware::bluetooth_audio::V2_0::CodecType;
...
using ::vendor::qti::hardware::bluetooth_audio::V2_1::IBluetoothAudioPort;
...
void ldac_codec_parser_2_0(AudioConfiguration_2_0*codec_cfg);
void aac_codec_parser(AudioConfiguration*codec_cfg, uint32_t *sample_freq);
void aac_codec_parser_2_0(AudioConfiguration_2_0*codec_cfg, uint32_t *sample_freq);
                               ...
                               uint32_t *sample_freq)
{
    ...
    if (codecConfig.codecType == CodecType::SBC) {
        ...
        *codec_type = AUDIO_FORMAT_AAC;
        aac_codec_parser(codec_cfg, sample_freq);
        ...
        *codec_type = static_cast<audio_format_t>(ENC_CODEC_TYPE_APTX_ADAPTIVE);
        ...
        if (!strcmp(adaptive_prop_value, "true")) {
          aptx_adaptive_codec.max_sink_buffering_LL = 2*codecConfig.config.aptxAdaptiveConfig.sinkBuffering.maxSinkBuff_LL;
          aptx_adaptive_codec.max_sink_buffering_HQ = 2*codecConfig.config.aptxAdaptiveConfig.sinkBuffering.maxSinkBuff_HQ;
          aptx_adaptive_codec.max_sink_buffering_TWS = 2*codecConfig.config.aptxAdaptiveConfig.sinkBuffering.maxSinkBuff_TWS;
        }
    ...
    } else if (codecConfig.codecType == CodecType::LC3) {
        ALOGE("%s: codec type is LC3",__func__);
        if (lc3_codec.enc_config.streamMapOut == NULL) {
            ALOGD("%s: Free allocated streamMap memory",__func__);
            free(lc3_codec.enc_config.streamMapOut);
            lc3_codec.enc_config.streamMapOut = NULL;
        }
        if (lc3_codec.dec_config.streamMapIn == NULL) {
            ALOGD("%s: Free allocated streamMap memory",__func__);
            free(lc3_codec.dec_config.streamMapIn);
            lc3_codec.dec_config.streamMapIn = NULL;
        }
        ...
        */
        ALOGE("%s: rxConfigSet: %d", __func__, codecConfig.config.lc3Config.rxConfigSet);

        switch (codecConfig.config.lc3Config.txConfig.sampleRate) {
            case ExtSampleRate::RATE_48000:
                 lc3_codec.enc_config.toAirConfig.sampling_freq = 48000;
                 break;
            case ExtSampleRate::RATE_44100:
                 lc3_codec.enc_config.toAirConfig.sampling_freq = 44100;
                 break;
            case ExtSampleRate::RATE_32000:
                 lc3_codec.enc_config.toAirConfig.sampling_freq = 32000;
                 break;
            case ExtSampleRate::RATE_24000_2:
                 lc3_codec.enc_config.toAirConfig.sampling_freq = 24000;
                 break;
            case ExtSampleRate::RATE_16000:
                 lc3_codec.enc_config.toAirConfig.sampling_freq = 16000;
                 break;
            case ExtSampleRate::RATE_8000:
                 lc3_codec.enc_config.toAirConfig.sampling_freq = 8000;
                 break;
            default:
                 ALOGE("%s: LC3: unknown sampling rate:%d", __func__,
                       (uint32_t)codecConfig.config.lc3Config.txConfig.sampleRate);
                 break;
        }
        lc3_codec.enc_config.toAirConfig.api_version = 0x00000021;
        lc3_codec.enc_config.toAirConfig.max_octets_per_frame =
                          codecConfig.config.lc3Config.txConfig.octetsPerFrame;
        lc3_codec.enc_config.toAirConfig.frame_duration =
                          codecConfig.config.lc3Config.txConfig.frameDuration;
        lc3_codec.enc_config.toAirConfig.bit_depth =
                          (uint32_t)codecConfig.config.lc3Config.txConfig.bitsPerSample;
        switch (codecConfig.config.lc3Config.txConfig.bitsPerSample) {
            case BitsPerSample::BITS_16:
                 lc3_codec.enc_config.toAirConfig.bit_depth = 16;
                 break;
            case BitsPerSample::BITS_24:
                 lc3_codec.enc_config.toAirConfig.bit_depth = 24;
                 break;
            case BitsPerSample::BITS_32:
                 lc3_codec.enc_config.toAirConfig.bit_depth = 32;
                 break;
            default:
                 ALOGE("%s: LC3: unknown bits per sample", __func__);
                 break;
        }
        lc3_codec.enc_config.toAirConfig.num_blocks =
                          codecConfig.config.lc3Config.txConfig.numBlocks;
        lc3_codec.enc_config.toAirConfig.mode = codecConfig.config.lc3Config.mode;
        for (int i = 0; i < 16; i++) {
            lc3_codec.enc_config.toAirConfig.vendor_specific[i] =
                   codecConfig.config.lc3Config.codecSpecific[i];
            ALOGD("%s: ## LC3Q ## vendor_specific encoder:[%d] = 0x%x",
               __func__, i, lc3_codec.enc_config.toAirConfig.vendor_specific[i]);
        }
        lc3_codec.enc_config.toAirConfig.default_q_level =
                     codecConfig.config.lc3Config.defaultQlevel;
        ALOGD("%s:LC3 Encr: SR = %d octets = %d, input_frame = %d, num_blocks = %d",
              __func__, lc3_codec.enc_config.toAirConfig.sampling_freq,
              lc3_codec.enc_config.toAirConfig.max_octets_per_frame,
              lc3_codec.enc_config.toAirConfig.frame_duration,
              lc3_codec.enc_config.toAirConfig.num_blocks);

        if ((codecConfig.config.lc3Config.rxConfigSet == STREAM_RX_ONLY_CONFIG) ||
            (codecConfig.config.lc3Config.rxConfigSet == STREAM_TX_RX_BOTH_CONFIG)) {
            switch (codecConfig.config.lc3Config.rxConfig.sampleRate) {
                case ExtSampleRate::RATE_48000:
                     lc3_codec.dec_config.fromAirConfig.sampling_freq = 48000;
                     break;
                case ExtSampleRate::RATE_44100:
                     lc3_codec.dec_config.fromAirConfig.sampling_freq = 44100;
                     break;
                case ExtSampleRate::RATE_32000:
                     lc3_codec.dec_config.fromAirConfig.sampling_freq = 32000;
                     break;
                case ExtSampleRate::RATE_24000_2:
                     lc3_codec.dec_config.fromAirConfig.sampling_freq = 24000;
                     break;
                case ExtSampleRate::RATE_16000:
                     lc3_codec.dec_config.fromAirConfig.sampling_freq = 16000;
                     break;
                case ExtSampleRate::RATE_8000:
                     lc3_codec.dec_config.fromAirConfig.sampling_freq = 8000;
                     break;
                default:
                     ALOGE("%s: LC3: unknown sampling rate:%d", __func__,
                       (uint32_t)codecConfig.config.lc3Config.txConfig.sampleRate);
            ...
            lc3_codec.dec_config.fromAirConfig.max_octets_per_frame =
                           codecConfig.config.lc3Config.rxConfig.octetsPerFrame;
            lc3_codec.dec_config.fromAirConfig.frame_duration =
                           codecConfig.config.lc3Config.rxConfig.frameDuration;
            switch (codecConfig.config.lc3Config.rxConfig.bitsPerSample) {
                case BitsPerSample::BITS_16:
                     lc3_codec.dec_config.fromAirConfig.bit_depth = 16;
                     break;
                case BitsPerSample::BITS_24:
                     lc3_codec.dec_config.fromAirConfig.bit_depth = 24;
                     break;
                case BitsPerSample::BITS_32:
                     lc3_codec.dec_config.fromAirConfig.bit_depth = 32;
                     break;
                default:
                     ALOGE("%s: LC3: unknown bits per sample", __func__);
                     break;
            ...
            lc3_codec.dec_config.fromAirConfig.num_blocks =
                           codecConfig.config.lc3Config.rxConfig.numBlocks;
            lc3_codec.dec_config.fromAirConfig.mode =
                           codecConfig.config.lc3Config.mode;
            for (int i = 0; i < 16; i++) {
                lc3_codec.dec_config.fromAirConfig.vendor_specific[i] =
                         codecConfig.config.lc3Config.codecSpecific[i];
                ALOGD("%s: ## LC3Q ## vendor_specific decoder:[%d] = 0x%x",
                  __func__, i, lc3_codec.dec_config.fromAirConfig.vendor_specific[i]);
            }
            lc3_codec.dec_config.fromAirConfig.default_q_level =
                                     codecConfig.config.lc3Config.defaultQlevel;
            ALOGE("%s:LC3 Dec: SR = %d octets = %d, input_frame = %d, num_blocks = %d",
        ...
        int in = 0;
        int out = 0;
        int stream_map_size = codecConfig.config.lc3Config.NumStreamIDGroup;

        if (codecConfig.config.lc3Config.rxConfigSet == STREAM_TX_ONLY_CONFIG) {
            out = stream_map_size;
        } else {//find number of out and in stream map
            for (int i = 0; i < stream_map_size * 3; i++) {
                if (codecConfig.config.lc3Config.streamMap[i+2] == toAir)
                    out++;
                else
                    in++;
                i += 2;
            }
            lc3_codec.dec_config.decoder_output_channel =
                            codecConfig.config.lc3Config.decoderOuputChannels;
        }

        ALOGD("%s: decoderOuputChannels: %d", __func__,
                    codecConfig.config.lc3Config.decoderOuputChannels);

        if (codecConfig.config.lc3Config.mode == BAP_BA_SIMULCAST) {
            lc3_codec.enc_config.stream_map_size = out/2;
            lc3_codec.dec_config.stream_map_size = out/2;
        } else {
            lc3_codec.enc_config.stream_map_size = out;
            lc3_codec.dec_config.stream_map_size = in;
        }

        ALOGD("%s: out stream map = %d, in stream map = %d", __func__,
        lc3_codec.enc_config.stream_map_size, lc3_codec.dec_config.stream_map_size);
        ...
        ALOGD("%s: parsing stream map",__func__);
        for (int i = 0; i < stream_map_size * 3;) {
            if (codecConfig.config.lc3Config.mode != BAP_BA_SIMULCAST) {
                if (codecConfig.config.lc3Config.streamMap[i+2] == 0) {
                    p_out->audio_location = codecConfig.config.lc3Config.streamMap[i++];
                    p_out->stream_id = (uint8_t)codecConfig.config.lc3Config.streamMap[i++];
                    p_out->direction = (uint8_t)codecConfig.config.lc3Config.streamMap[i++];
                    ...
                    p_in->audio_location = codecConfig.config.lc3Config.streamMap[i++];
                    p_in->stream_id = (uint8_t)codecConfig.config.lc3Config.streamMap[i++];
                    p_in->direction = (uint8_t)codecConfig.config.lc3Config.streamMap[i++];
                ...
                if (i < lc3_codec.enc_config.stream_map_size * 3) {
                    p_out->audio_location = codecConfig.config.lc3Config.streamMap[i++];
                    p_out->stream_id = (uint8_t)codecConfig.config.lc3Config.streamMap[i++];
                    p_out->direction = (uint8_t)codecConfig.config.lc3Config.streamMap[i++];
                    ALOGD("%s: stream_map p_out: Simulcast stream_id = %d, audio location = %d,"
                               " direction = %d", __func__, p_out->stream_id,
                               p_out->audio_location, p_out->direction);
                    p_out++;
                } else  {
                    ...
                    p_in->stream_id = (uint8_t)codecConfig.config.lc3Config.streamMap[i++];
                    p_in->direction = (uint8_t)codecConfig.config.lc3Config.streamMap[i++];
                    ALOGD("%s: stream_map p_in: Simulcast stream_id = %d, audio location = %d,"
                               " direction = %d", __func__, p_in->stream_id,
                               p_in->audio_location, p_in->direction);
                    p_in++;
                }
        ...
        } else if (codecConfig.config.lc3Config.rxConfigSet == STREAM_RX_ONLY_CONFIG) {
          lc3_codec.is_enc_config_set = false;
          lc3_codec.is_dec_config_set = true;
        } else if (codecConfig.config.lc3Config.rxConfigSet == STREAM_TX_RX_BOTH_CONFIG) {
          lc3_codec.is_enc_config_set = true;
          lc3_codec.is_dec_config_set = true;
        }

        ALOGD("%s: is_enc_config_set: %d, is_dec_config_set: %d",
               __func__, lc3_codec.is_enc_config_set, lc3_codec.is_dec_config_set);

       *codec_type = static_cast<audio_format_t>(AUDIO_CODEC_TYPE_LC3);
        return ((void*)&lc3_codec);
    }
...
static void* a2dp_codec_parser_2_0(AudioConfiguration_2_0*codec_cfg,
                               audio_format_t *codec_type,
                               uint32_t *sample_freq)
{
    CodecConfiguration_2_0 codecConfig =  codec_cfg->codecConfig;
    ALOGI("%s: codec_type = %x", __func__, codecConfig.codecType);
    if (codecConfig.codecType == CodecType_2_0::SBC) {
        memset(&sbc_codec, 0, sizeof(audio_sbc_encoder_config_t));
    ...
    } else if (codecConfig.codecType == CodecType_2_0::AAC) {
        *codec_type = AUDIO_FORMAT_AAC;
        aac_codec_parser_2_0(codec_cfg, sample_freq);
        return ((void *)(&aac_codec));
    } else if (codecConfig.codecType == CodecType_2_0::APTX ||
               codecConfig.codecType == CodecType_2_0::APTX_HD) {
        memset(&aptx_codec, 0, sizeof(audio_aptx_encoder_config_t));
        ...
        if (codecConfig.codecType == CodecType_2_0::APTX) {
           *codec_type = AUDIO_FORMAT_APTX;
           ALOGI("%s: APTX: done copying full codec config", __func__);
        } else if (codecConfig.codecType == CodecType_2_0::APTX_HD) {
           *codec_type = AUDIO_FORMAT_APTX_HD;
           ALOGI("%s: APTXHD: done copying full codec config", __func__);
        }
    ...
    } else if (codecConfig.codecType == CodecType_2_0::LDAC) {
        *codec_type = AUDIO_FORMAT_LDAC;
        ldac_codec_parser_2_0(codec_cfg);
        return ((void *)&ldac_codec);
    } else if (codecConfig.codecType == CodecType_2_0::APTX_ADAPTIVE) {
        *codec_type = static_cast<audio_format_t>(ENC_CODEC_TYPE_APTX_ADAPTIVE);
        memset(&aptx_adaptive_codec, 0, sizeof(audio_aptx_adaptive_encoder_config_t));
        ...
        //memcpy(aptx_adaptive_codec.sink_capabilities,
                ...
                //sizeof(aptx_adaptive_codec.sink_capabilities));
        return ((void *)&aptx_adaptive_codec);
    } else if (codecConfig.codecType == CodecType_2_0::APTX_TWS) {
        *codec_type = static_cast<audio_format_t>(ENC_CODEC_TYPE_APTX_DUAL_MONO);
        ...
        return ((void *)&aptx_tws_codec);
    } else if (codecConfig.codecType == CodecType_2_0::BA_CELT) {
       //TODO not tested
        *codec_type = static_cast<audio_format_t>(AUDIO_CODEC_TYPE_CELT);
  ...
  //  if (cntrl_key == audio_stream.ctrl_key)  {
                     ...
                     || audio_stream.state == AUDIO_A2DP_STATE_STARTING)
       {
         ...
         ALOGI("%s:  end session session ready  = %d", __func__, audio_stream.session_ready);
          ...
          memset(&audio_stream.codec_cfg_2_0, 0, sizeof(CodecConfiguration_2_0));
          if (BluetoothAudioSessionControl::isClient_2_0(audio_stream.sessionType)) {
            audio_stream.codec_cfg_2_0 =
                        BluetoothAudioSessionControl::GetAudioConfig_2_0(audio_stream.sessionType);
          } else {
            audio_stream.codec_cfg =
                        BluetoothAudioSessionControl::GetAudioConfig(audio_stream.sessionType);
          }
       }
...
static void btapoffload_port_init()
{
    ...
    ::vendor::qti::bluetooth_audio::V2_1::PortStatusCallbacks cbacks = {
...
int audio_stream_start_qti()
{
    ...
    ALOGD("%s: rxConfigSet: %d", __func__,
                audio_stream.codec_cfg.codecConfig.config.lc3Config.rxConfigSet);

    if (((audio_stream.codec_cfg.codecConfig.config.lc3Config.rxConfigSet ==
                                                   STREAM_TX_ONLY_CONFIG) &&
         (audio_stream.state == AUDIO_A2DP_STATE_SUSPENDED)) ||
        ((audio_stream.codec_cfg.codecConfig.config.lc3Config.rxConfigSet ==
                                                   STREAM_RX_ONLY_CONFIG) &&
         (audio_stream.state == AUDIO_A2DP_STATE_STARTED))) {
        ALOGW("%s: stream suspended or "
                  "stream already in started state, return", __func__);
...
bool bt_audio_pre_init_qti()
{
    ...
    if(!hal_registered) {
      bool fail = android::hardware::registerPassthroughServiceImplementation
          ...
          V2_1::IBluetoothAudioProvidersFactory>() != android::OK;
      if(fail) {
        ALOGE("%s: HIDL IBluetoothAudioProvidersFactory 2.1 registration failed", __func__);
      } else {
        ALOGI("%s: IBluetoothAudioProvidersFactory 2.1 registration succeded", __func__);
        hal_registered = true;
      }
    } else {
    ...
    pthread_mutex_unlock(&audio_stream.lock);
    return hal_registered;
}

int audio_stream_open_qti()
{
    ALOGI("%s", __func__);
    if (!hal_registered) return 0;
    int ret = 0;
    ...
    for (int i = 0; i < STREAM_OPEN_RETRIAL_COUNT; i++) {
        ...
        if (audio_stream.session_ready) {
            if (BluetoothAudioSessionControl::isClient_2_0(audio_stream.sessionType) == false) {
                ALOGI("%s: audio stream hal is 2_1", __func__);
                audio_stream.hal_2_1 = true;
            }
            ...
            if (BluetoothAudioSessionControl::IsSessionReady(
                ...
                ALOGI("%s: SessionType is sw encoding.",__func__);
                ret = -1;
                break;
            } else {
                              ...
                              audio_format_t *codec_type)
{
    ALOGI("%s: state = %s", __func__, dump_a2dp_hal_state(audio_stream.state));
    if (audio_stream.session_ready) {
      if (audio_stream.hal_2_1)
        return (a2dp_codec_parser(&audio_stream.codec_cfg, codec_type, NULL));
      else
        return (a2dp_codec_parser_2_0(&audio_stream.codec_cfg_2_0, codec_type, NULL));
    }
    ALOGW("%s: return", __func__);
    return NULL;
}
...
uint16_t audio_get_a2dp_sink_latency_qti()
{
    ...
    if (audio_stream.session_ready) {
        BluetoothAudioSessionControl::a2dpGetSinkLatency(audio_stream.sessionType,
                &remoteDelay, &total_bytes_readed, data_position);
...
void aac_codec_parser(AudioConfiguration *codec_cfg, uint32_t *sample_freq)
{
    ...
    CodecConfiguration codecConfig =  codec_cfg->codecConfig;
    const AacData aacconfig =   codecConfig.config.aacConfig;
    if (aac_codec.frame_ptr_ctl != NULL) {
      ALOGE("%s: Free allocated memory", __func__);
      free(aac_codec.frame_ptr_ctl);
      aac_codec.frame_ptr_ctl = NULL;
    }
    ...
    switch (aacconfig.variableBitRateEnabled) {
        case AacVariableBitRate::ENABLED:
    ...
    aac_codec.abr_size_control_struct = A2D_AAC_SIZE_CTL_STRUCT;
...
//Sink stop
int audio_sink_stream_stop_qti()
{
    ALOGI("%s", __func__);
    int ret = 0;
    tA2DP_CTRL_ACK status = A2DP_CTRL_ACK_UNKNOWN;
    ...
    if (audio_stream.codec_cfg.codecConfig.config.lc3Config.rxConfigSet ==
                                                 STREAM_TX_RX_BOTH_CONFIG) {
        //Ignore when both Gaming Tx and Rx is in streaming.
        //stop only when Game Tx would exit.
        ALOGW("%s: Ignore Sink stop and ack success", __func__);
        pthread_mutex_unlock(&audio_stream.lock);
...
//Sink Suspend
int audio_sink_stream_suspend_qti()
{
    ALOGI("%s", __func__);
    tA2DP_CTRL_ACK status;
    ...
    if (audio_stream.codec_cfg.codecConfig.config.lc3Config.rxConfigSet ==
                                                 STREAM_TX_RX_BOTH_CONFIG) {
        //Ignore when both Gaming Tx and Rx is in streaming.
        //suspend only when Game Tx would exit.
        ALOGW("%s: Ignore Sink suspend and ack success", __func__);
        pthread_mutex_unlock(&audio_stream.lock);
        return A2DP_CTRL_ACK_SUCCESS;
    }
...
//Sink start
int audio_sink_stream_start_qti()
{
    int retry_count = 0;
    int wait_time = 0;
    tA2DP_CTRL_ACK status = A2DP_CTRL_ACK_UNKNOWN;
