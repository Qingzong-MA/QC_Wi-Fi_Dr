...
ndk::ScopedAStatus BluetoothAudioProvider::endSession() {
  ...
  if ((stack_iface_ != nullptr) && (!is_binder_died)) {
...
void BluetoothAudioProvider::binderDiedCallbackAidl(void* ptr) {
  ...
  LOG(ERROR) << __func__ << " - death_recipient_.get()=" << provider->death_recipient_.get();
  if (provider->death_recipient_.get() == nullptr) {
    LOG(ERROR) << __func__ << " - SessionType= "<< toString(provider->session_type_);
    BluetoothAudioSessionReport::OnSessionEnded(provider->session_type_);
    provider->stack_iface_ = nullptr;
    provider->audio_config_ = nullptr;
    return;
  }
  provider->endSession();
}
