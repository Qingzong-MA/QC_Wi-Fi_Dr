...
void BluetoothAudioSession::OnSessionStarted(
  ...
  std::lock_guard<std::recursive_mutex> guard(stack_ops_mutex_);
...
void BluetoothAudioSession::ReportAudioConfigChanged(
  ...
  std::lock_guard<std::recursive_mutex> guard(hal_ops_mutex_);
  ...
  for (auto& observer : observers_) {
    ...
    if (cb->audio_configuration_changed_cb_ != nullptr) {
      cb->audio_configuration_changed_cb_(cookie, audio_config);
    }
  }
}
...
bool BluetoothAudioSession::UpdateAudioConfig(
  ...
  bool is_le_audio_broadcast_offload_audio_config =
      (is_offload_le_audio_broadcast_session &&
       audio_config_tag == AudioConfiguration::leAudioBroadcastConfig);
  if (!is_software_audio_config && !is_a2dp_offload_audio_config &&
      !is_le_audio_offload_audio_config && !is_le_audio_broadcast_offload_audio_config) {
    return false;
  }
  audio_config_ = std::make_unique<AudioConfiguration>(audio_config);
  return true;
...
void BluetoothAudioSession::ReportSessionEndedStatus() {
  // Remove observer here and so that Audio HAL doesn't need to
  // callback to remove the observer in the same thread
  std::lock_guard<std::recursive_mutex> guard(hal_ops_mutex_);
  if (observers_.empty()) {
    LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
              << " has NO port state observer";
  ...
  while(observer != observers_.end()) {
    ...
    LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
              ...
              << ::android::base::StringPrintf("%04x", cookie);
    callback->session_changed_cb_(cookie);
    observer = observers_.erase(observer);
  }
}
...
size_t BluetoothAudioSession::OutWritePcmData(const void* buffer,
  ...
  do {
    std::unique_lock<std::recursive_mutex> lock(stack_ops_mutex_);
    if (!IsSessionReady()) {
      break;
    }
