...
struct PortStatusCallbacks {
   ...
   * @param: cookie - indicates which bluetooth_audio output should handle
   ***/
  std::function<void(uint16_t cookie, const AudioConfiguration& audio_config)>
                                                audio_configuration_changed_cb_;
  /***
...
class BluetoothAudioSession {
 ...
 private:
  ...
  std::recursive_mutex stack_ops_mutex_;
  std::recursive_mutex hal_ops_mutex_;
